...
def completion_with_backoff(client, model, messages, timeout, temperature):
    return client.chat.completions.create(model=model, messages=messages, timeout=timeout, temperature=temperature)
...
def get_response(prompt_token_size, temperature, max_response_size, messages, timeout=60):
    ...
    if model == "GPT-3.5":
        if prompt_token_size < 16000:
            ...
            raise NotImplementedError(
                "Error - context windows longer than 16K tokens are not supported by GPT-3.5 model")
    elif model == "GPT-4":
        if prompt_token_size > 8000:
            raise NotImplementedError(
                "Error - input texts with more than 8000 tokens are not supported by currently available models")
        else:
    ...
    response = completion_with_backoff(
        client=client, model=selected_model, messages=messages, timeout=timeout, temperature=temperature
    )
...
def prompt(user_prompt, temperature=0, max_response_size=300, timeout=600):
